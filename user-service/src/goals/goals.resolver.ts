...
export class GoalsResolver {
  ...
  @Mutation(() => Goals, { description: 'Save a goal, Ex: saveGoal(saveGoalDto: { name: "Goal 1", description: "Description 1", tasks: [{ name: "Task 1", link: "https://example.com", completed: true }] })' })
  async saveGoal(
  ...
  @Mutation(() => Boolean, { description: 'Delete a goal, Ex: deleteGoal(id: "1234567890")' })
  async deleteGoal(
    @Context('userId') userId: Types.ObjectId,
    @Args('id', { type: () => ID, nullable: true }) id?: Types.ObjectId,
  ) {
  ...
  @Mutation(() => ToggleTaskResponseDto, { description: 'Toggle a task, Ex: toggleTask(goalId: "1234567890", taskId: 1)' })
  async toggleTask(
    @Context('userId') userId: Types.ObjectId,
  ...
  @Query(() => GoalSummaryResponse, { description: 'Get a goal summary (admin only), Ex: adminGetGoalSummary(goalSummaryInput: { period: "WEEK" })' })
